...
settingsModal.innerHTML = `
  <div class="picker-box">
    ...
    <p class="muted" style="font-size:13px; margin-top:6px">Тема оформления</p>
    <div class="cur-row">
      <button class="chip${getTheme() === "light" ? " on" : ""}" data-theme-set="light">☀ Светлая</button>
      <button class="chip${getTheme() === "dark" ? " on" : ""}" data-theme-set="dark">☾ Тёмная</button>
    </div>
...
function settingsFx() {
  ...
  settingsModal.querySelectorAll("[data-theme-set]").forEach((b) => {
    b.addEventListener("click", () => {
      setTheme(b.dataset.themeSet);
      settingsModal.querySelectorAll("[data-theme-set]").forEach((x) => x.classList.toggle("on", x === b));
...
// бегущая строка-волна: текст по SVG-кривой (как React Bits TextLoop),
// два экземпляра textPath для бесшовного цикла + лента, плавное замедление на ховер
function textLoopMarquee(root, phrase, opts = {}) {
  const NS = "http://www.w3.org/2000/svg";
  const VW = 1200, VH = 140, CY = VH / 2;
  const amp = opts.amp ?? 26;
  const sep = opts.separator ?? "  ·  ";
  const baseSpeed = opts.speed ?? 80;
  const ribW = opts.ribbonWidth ?? 54;
  const ribColor = opts.ribbonColor || "#211d16";
  const textColor = opts.textColor || "#fbf8f0";

  const unit = phrase + sep;
  const d = `M -320 ${CY} Q -160 ${CY - amp} 0 ${CY} T 320 ${CY} T 640 ${CY} T 960 ${CY} T 1280 ${CY} T ${VW + 320} ${CY}`;
  ...
  const pid = "mq-" + Math.random().toString(36).slice(2, 8);
  const path = document.createElementNS(NS, "path");
  path.setAttribute("d", d);
  path.setAttribute("fill", "none");
  path.setAttribute("class", "marquee-ribbon");
  path.setAttribute("stroke-width", ribW);
  path.setAttribute("stroke-linecap", "round");
  path.setAttribute("id", pid);
  svg.append(path);

  const measure = document.createElementNS(NS, "text");
  measure.setAttribute("class", "marquee-measure");
  measure.textContent = unit;
  svg.append(measure);

  const mkTextPath = () => {
    const t = document.createElementNS(NS, "text");
    t.setAttribute("class", "marquee-txt");
  ...
  const head = mkTextPath();
  const tail = mkTextPath();
  root.append(svg);

  let raf = 0, offset = 0, last = 0, speed = 1, target = 1, len = 0, ready = false;
  const apply = (o) => {
    const partner = o >= 0 ? o - len : o + len;
  ...
  const tick = (now) => {
    ...
    offset = (offset + speed * baseSpeed * dt) % len;
    apply(offset);
  ...
  const setup = () => {
    ...
    const reps = Math.max(1, Math.floor(length / unitW));
    ...
    if (!ready) {
      ready = true;
      apply(0);
      if (!opts.calm) raf = requestAnimationFrame(tick);
    }
  };
  root.addEventListener("mouseenter", () => (target = 0.03));
  root.addEventListener("mouseleave", () => (target = 1));
  setup();
...
async function homeInit() {
  ...
  const mq = $("#marquee");
  if (mq && !mq.dataset.init) {
    mq.dataset.init = "1";
    const phrase = ["EAZYTECH", ...brands].join("  ·  ");
    textLoopMarquee(mq, phrase, { calm: CALM });
  }
}
